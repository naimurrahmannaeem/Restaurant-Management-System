...
### Customer Side
- Browse full menu with category filter
- Add items to cart (AJAX, no page reload)
- Adjust quantity / remove from cart
- Checkout with delivery address, order type, payment method
- Free delivery on orders ৳500+
- View order history with live status
- Leave star ratings & comments after delivery

### Admin Panel
- Dashboard with 8 live stat cards
- Manage orders — update status (pending → confirmed → preparing → out for delivery → delivered)
- Full menu CRUD (add/edit/delete/toggle availability)
- Category management
- Customer directory with search
- Review moderation
- Revenue reports with date range filter (daily breakdown, by category, by payment method)
- Multi-admin management with roles (superadmin / manager / staff)
